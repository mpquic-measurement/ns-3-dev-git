...
QuicSocketBase::SendDataPacket (SequenceNumber32 packetNumber, uint32_t maxSize, bool withAck, uint8_t pathId)
{
  ...
  if (m_socketState == CONNECTING_SVR)
    ...
    {
      std::cout << __FILE__ << ": " << __LINE__ << " on creating quic header, cid: " << m_connectionId << std::endl;
...
{
  NS_LOG_FUNCTION (this);
  std::cout << __FILE__ << ": " << __LINE__ << " QuicSocketBase::SetVersion: " << version << std::endl;
  m_vers = version;
  return;
}
                                      ...
                                      Ptr<Packet> packet)
{
  ...
  std::cout << __FILE__ << ": " << __LINE__ << " initial handshake type: " << int(type) << std::endl;
  
  if (type == QuicHeader::VERSION_NEGOTIATION)
    {
      std::cout << __FILE__ << ": " << __LINE__ << " create VERSION_NEGOTIATION" << std::endl;
    ...
    {
      std::cout << __FILE__ << ": " << __LINE__ << " create INITIAL" << std::endl;
      std::cout << __FILE__ << ": " << __LINE__ << " version: " << m_vers << std::endl;
      //client(sender)
      // Set initial congestion window and Ssthresh
      m_subflows[0]->m_tcb->m_cWnd = m_subflows[0]->m_tcb->m_initialCWnd;
      m_subflows[0]->m_tcb->m_ssThresh = m_subflows[0]->m_tcb->m_initialSsThresh;
    ...
    {
      std::cout << __FILE__ << ": " << __LINE__ << " wrong handshake type" << std::endl;
...
QuicSocketBase::DoConnect (void)
{
  ...
  if (m_socketState == LISTENING)
  ...
  else if (m_socketState == IDLE)
    {
      ...
      std::cout << __FILE__ << ": " << __LINE__ << " sending QUIC initial handshake" << std::endl;
      QuicHeader q;
